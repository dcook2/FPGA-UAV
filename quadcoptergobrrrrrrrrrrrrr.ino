...
float roll_p_gain = 1.3;    //original 1.3, 0.04, 18.0
float roll_i_gain = 0.03;
float roll_d_gain = 18.0;
int roll_max = 400;       //original 400
...
float yaw_p_gain = 4.0;   //original 4.0, 0.02, 0
float yaw_i_gain = 0.02;
float yaw_d_gain = 0;
int yaw_max = 400;      //original 400
...
sensors_event_t event_gyro, event_orientation;
...
void setup() {
  ...
  for(count = 0; count < 2000; count++){
    ...
    gyro_avg[0] += event_gyro.gyro.x;
    gyro_avg[1] += event_orientation.orientation.y;
    gyro_avg[2] += event_orientation.orientation.z;
    delay(3);
  }
...
//    Serial.print(map(analogRead(A0),0,1023,1000,1120));
//    //Serial.print(throttle);
...
//    delay(50);
    ...
    switch(currentState)
    {
      ...
      throttle = map(throttle,0,1023,1000,1120);
      for(int i = 0; i < 4; i++){
        motor[i].writeMicroseconds(throttle);
      }
      delay(15);
...
//      yaw_d = 0;
      ...
      motor[0].writeMicroseconds(constrain(FR_THROTTLE,1000,1150));
      motor[1].writeMicroseconds(constrain(FL_THROTTLE,1000,1150));
      motor[2].writeMicroseconds(constrain(BR_THROTTLE,1000,1150));
      motor[3].writeMicroseconds(constrain(BL_THROTTLE,1000,1150));
      }
    ...
    Serial.print(event_orientation.orientation.y);
    Serial.print("    orient z");
    Serial.println(event_orientation.orientation.z);
    Serial.print(   "gyro y");
    Serial.print(event_gyro.gyro.y);
    delay(50);
...
void PID() {
  ...
  yaw_input = event_gyro.gyro.x - gyro_avg[0];
  pitch_input = event_orientation.orientation.y - gyro_avg[1];
  roll_input = event_orientation.orientation.z - gyro_avg[2];
